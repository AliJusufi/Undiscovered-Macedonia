...
http
      ...
      (async () => {
        try {
          const payload = {
            ...
            contents: [
              { role: "user", parts: [{ text: "Where should I go first in Macedonia?" }] },
            ],
          ...
          res.writeHead(200, { ...cors, "content-type": "application/json" });
          res.end(
            JSON.stringify({ httpStatus: r.status, webSearch: USE_WEB_SEARCH, raw: data }, null, 2)
          );
        } catch (e) {
          res.writeHead(200, { ...cors, "content-type": "application/json" });
          res.end(JSON.stringify({ networkError: String(e) }, null, 2));
  ...
  .listen(PORT, () =>
    console.log("Gemini AI proxy on http://localhost:" + PORT + "  (model: " + MODEL + ")")
  );
