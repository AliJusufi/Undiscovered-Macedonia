...
(function () {
  ...
  var SEED = [
    ...
    },
    {
      ...
      country: "Portugal",
      flag: "🇵🇹",
      langs: ["Portuguese", "Spanish", "English"],
      pace: "Relaxed",
  ...
  function sharedInterests(p) {
    ...
    return p.interests.filter(function (x) {
      return profile.interests.indexOf(x) !== -1;
    });
  }
  ...
  function buildDeck() {
    queue = SEED.filter(function (p) {
      return !seen[p.id];
    });
    // most-in-common first
    queue.sort(function (a, b) {
      return sharedInterests(b).length - sharedInterests(a).length;
    });
    renderDeck();
  }
  ...
  function renderDeck() {
    ...
    if (!profile) {
      deck.innerHTML =
        '<p class="tv-empty" id="deckLocked">Save your profile to see other travellers.</p>';
  ...
  function cardEl(p, isTop) {
    ...
    var tags = p.interests
      .map(function (i) {
        return (
          '<span class="tv-tag' + (shared.indexOf(i) !== -1 ? " shared" : "") + '">' + i + "</span>"
        );
      })
      .join("");
  ...
  function openingLine(p) {
    ...
    if (sh.length)
      return "Hey! Saw we're both into " + sh[0].toLowerCase() + " — when are you around?";
  ...
  function replyTo(text, p) {
    ...
    if (q.indexOf("ohrid") !== -1)
      return "I'll be in Ohrid too. Kaneo church at sunset is supposed to be the spot.";
    if (q.indexOf("skopje") !== -1) return "I'm in Skopje until Thursday. Free most afternoons.";
    if (q.indexOf("matka") !== -1 || q.indexOf("kayak") !== -1)
      return "Matka kayaking sounds great. Morning is calmer on the water I think.";
    if (/\b(when|what day|which day|time|date)\b/.test(q))
      return "I'm flexible — mornings are better for me. What suits you?";
    if (/\b(yes|yeah|sure|sounds good|let's|lets|deal|ok|okay)\b/.test(q))
      return "Perfect. I'll message you here to sort the details.";
    if (/\b(hi|hey|hello|yo)\b/.test(q))
      return "Hey " + (profile ? profile.name : "there") + "! Good to match.";
    if (q.indexOf("?") !== -1)
      return (
        "Good question — let me check and get back to you. " +
        (sh.length ? "Either way, down for something " + sh[0].toLowerCase() + "-related." : "")
      );
    return sh.length
      ? "Nice. Since we're both into " +
          sh[0].toLowerCase() +
          ", want to plan something around that?"
