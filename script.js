...
(function () {
  ...
  function openModal(title, sub, buildBody) {
    ...
    wrap.innerHTML =
      '<div class="um-modal-card" role="dialog" aria-modal="true" aria-label="' + title + '">' +
    ...
    buildBody(wrap.querySelector(".um-modal-content"), function () { wrap.remove(); });
  ...
  if (planForm) {
    planForm.addEventListener("submit", function (e) {
      ...
      document.getElementById("planMsg").textContent =
        "Saved as " + ref + ". On a live site this would reach our guides; here it's kept in your browser only.";
      f.reset();
    });
  }
  ...
  if (viewEnq) {
    viewEnq.addEventListener("click", function (e) {
      e.preventDefault();
      openModal("Saved enquiries", "Stored in this browser only — nothing was sent.", function (content, close) {
  ...
  function renderEnquiries(content, close) {
    var all = store.get("um_enquiries", []);
    if (!all.length) { content.innerHTML = '<p class="um-empty">No enquiries saved yet.</p>'; return; }
    var ul = document.createElement("ul");
    ul.className = "um-list";
    all.slice().reverse().forEach(function (en) {
      ...
      li.querySelector(".um-body").textContent =
        en.name + " · " + en.email + "\n" +
        (en.when ? "When: " + en.when + "   " : "") + (en.days ? "Days: " + en.days : "") +
        (en.notes ? "\n" + en.notes : "");
      li.querySelector(".um-del").addEventListener("click", function () {
        var rest = store.get("um_enquiries", []).filter(function (x) { return x.ref !== en.ref; });
        store.set("um_enquiries", rest);
        close(); openModal("Saved enquiries", "Stored in this browser only — nothing was sent.", renderEnquiries);
      });
      ul.appendChild(li);
    });
  ...
  var KB = [
    ...
    {
      k: ["skopje", "capital", "stone bridge", "bazaar", "earthquake", "mother teresa", "vodno", "millennium cross"],
    ...
    {
      k: ["stobi", "roman", "ruins", "archaeolog", "mosaic", "heraclea", "skupi", "kokino", "kuklica", "stone dolls", "observatory"],
    ...
    {
      k: ["wedding", "galicnik", "galičnik", "costume", "tradition", "carnival", "folklore", "custom"],
    ...
    {
      k: ["guide", "guides", "who leads", "tour leader", "marko", "petar", "ardit", "jon", "ana", "sara"],
    ...
    {
      k: ["meet", "other travellers", "other travelers", "companion", "match", "solo", "alone", "people", "friends"],
  ...
  /* ---------- Widget DOM ---------- */
  var messages = [
    { role: "assistant", content: "Hi, I'm UMac. Ask me anything about a trip to Macedonia — where to go, when, food, or planning." }
  ];
  ...
  fab.addEventListener("click", function () { openAsk(); });
  ...
  function submit(text) {
    ...
    respond(text).then(function (reply) {
      ...
      thinking.textContent = "Sorry — something went wrong. Try again, or email hello@undiscovered-macedonia.example.";
    });
  }
  ...
  function renderSavedTrips(content, close) {
    var all = store.get("um_saved_trips", []);
    if (!all.length) { content.innerHTML = '<p class="um-empty">Nothing saved yet. Ask UMac for an itinerary, then hit “Save to my trip”.</p>'; return; }
    var ul = document.createElement("ul");
    ul.className = "um-list";
    all.slice().reverse().forEach(function (t, i) {
      var li = document.createElement("li");
      li.innerHTML = '<p class="um-meta"></p><p class="um-body"></p><button class="um-del">Remove</button>';
      li.querySelector(".um-meta").textContent = "“" + t.q + "” · " + new Date(t.at).toLocaleDateString();
      ...
      li.querySelector(".um-del").addEventListener("click", function () {
        ...
        close(); openModal("Saved trips", "Answers you kept from UMac — stored in this browser.", renderSavedTrips);
      });
      ul.appendChild(li);
    });
    content.appendChild(ul);
  ...
  savedBtn.addEventListener("click", function () {
    openModal("Saved trips", "Answers you kept from UMac — stored in this browser.", renderSavedTrips);
  });

  function respond(text) {
    if (ASSISTANT_CONFIG.apiUrl) {
      return fetch(ASSISTANT_CONFIG.apiUrl, {
        method: "POST",
        ...
        .then(function (data) { return data.reply || localAnswer(text); })
    ...
    return new Promise(function (resolve) {
      setTimeout(function () { resolve(localAnswer(text)); }, 400);
    });
  }
  ...
  if (mapEl && window.L) {
    ...
    if ("IntersectionObserver" in window) {
      var mio = new IntersectionObserver(function (entries) {
        if (entries[0].isIntersecting) { buildMap(); mio.disconnect(); }
      }, { rootMargin: "300px" });
      mio.observe(mapEl);
    } else {
      buildMap();
  ...
  function initMap() {
    var mapEl = document.getElementById("macedonia-map");
    var PLACES = [
      { n: "Skopje", ll: [41.9981, 21.4254], d: "The capital — Ottoman bazaar, Stone Bridge, Mother Teresa's birthplace." },
      { n: "Matka Canyon", ll: [41.9556, 21.3025], d: "Kayaking and cave boat trips, 30 minutes from Skopje." },
      { n: "Ohrid", ll: [41.1231, 20.8016], d: "UNESCO lake town — St. John at Kaneo, frescoed churches, clear water." },
      { n: "Struga", ll: [41.1775, 20.6781], d: "Where the Black Drim leaves Lake Ohrid — beaches and the poetry festival." },
      { n: "Mavrovo", ll: [41.6875, 20.7469], d: "National park, reservoir lake and the main ski resort.", park: true },
      { n: "Bitola", ll: [41.0314, 21.3347], d: "The City of Consuls — café culture and Roman Heraclea nearby." },
      { n: "Pelister", ll: [40.9847, 21.2136], d: "National park on Baba Mountain — glacial lakes and molika pine.", park: true },
      { n: "Kruševo", ll: [41.3697, 21.2494], d: "The highest town in the country — paragliding and mountain air." },
      { n: "Popova Šapka", ll: [42.0072, 20.9550], d: "Ski and hiking resort on the Šar Mountains plateau." },
      { n: "Tetovo", ll: [42.0106, 20.9714], d: "Home of the painted Šarena Džamija mosque." },
      { n: "Prilep", ll: [41.3453, 21.5544], d: "Beneath Marko's Towers — tobacco, marble, Treskavec monastery." },
      { n: "Strumica", ll: [41.4378, 22.6431], d: "Spring carnival, Kolešino waterfall, the best peppers for ajvar." },
      { n: "Dojran", ll: [41.1889, 22.7167], d: "A warm, shallow lake on the Greek border." },
      { n: "Kratovo", ll: [42.0781, 22.1783], d: "A town inside an extinct volcano crater, linked by stone bridges." },
      { n: "Berovo", ll: [41.7072, 22.8558], d: "Pine forest and a mountain lake in the eastern Maleševo highlands." },
      { n: "Vevčani", ll: [41.2408, 20.5906], d: "Spring-fed village near Ohrid, famous for its winter carnival." },
      { n: "Stobi", ll: [41.5539, 21.9756], d: "Roman and early-Byzantine ruins with mosaics, in the Tikveš wine region." },
      { n: "Lake Prespa", ll: [40.9000, 20.9800], d: "A high lake shared with Greece and Albania — orchards and pelicans.", park: true },
      { n: "Galičica NP", ll: [40.9500, 20.8300], d: "The karst ridge between Ohrid and Prespa — panoramic road, wild horses.", park: true }
    ];

    var map = L.map(mapEl, { scrollWheelZoom: false, attributionControl: true })
      .setView([41.55, 21.5], 8);
